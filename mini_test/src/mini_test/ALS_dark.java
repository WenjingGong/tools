...
public class ALS_dark extends FeatureIm{
	...
	public static void serialcomm(String num){
		try {
			...
			System.out.println("In 1 second, please block the light:");
			Thread.sleep(1000);
			print(edit,"cd /usr/lib/leeonl/sensors");
			Thread.sleep(500);
			print(edit,"./get_light.rb");
			Thread.sleep(500);
			System.out.println("Now you can remove the light block.");
	...
	// Process the file and generate a standardized file
			public static boolean fileprocess(String downloadpath, String filepath, String []paras){
				...
				try {				
					...
					if (Integer.parseInt(content_first.get(1))>Integer.parseInt(paras[1])){
						throw new als_exception(3);
					}
					
					if (Double.parseDouble(content_first.get(2))>Double.parseDouble(paras[2])){
				...
				try {
					...
					bw.write("ALS_Dark_Value="+Double.parseDouble(content_first.get(2))+"\r\n");
					System.out.println("ALS_Dark_Value="+Double.parseDouble(content_first.get(2)));
					bw.write("ALS_Dark_Result="+result+"\r\n");
					System.out.println("ALS_Dark_Result="+result);
