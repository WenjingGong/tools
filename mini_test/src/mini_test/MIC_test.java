...
public class MIC_test extends FeatureIm{
	...
	public static void process(String num, String downloadpath, String filepath, String []paras, String sn){
		...
		for (int times = 0; times < CHUNK_SIZE; times ++){
			...
			for (int times = 0; times < CHUNK_SIZE/2;times++){
				...
				if (Math.abs(mag-content[times])/Math.abs(content[times])<Double.parseDouble(paras[0])){
					count++;
				}
			}
			
			float correct_proportion = (float)count/(CHUNK_SIZE/2);
			if (correct_proportion>Double.parseDouble(paras[1])){
				result = "PASS";
			}
			else {
				result = "FAILED";
	...
	public static void serialcomm(String num, String []paras){
		try {
			...
			Thread.sleep(500);
			print(edit,"jack_rec -f alarm.wav -d 5 -b 44100 system:capture_1");
